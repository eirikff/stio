...
namespace svo
{
  ...
  Frame::Frame(
      ...
      : id_(frame_counter_++) // TEMPORARY
        ,
        cam_(cam), key_pts_(5, std::make_pair(-1, BearingVector())), timestamp_(timestamp_ns)
  {
  ...
  Frame::Frame(
      ...
      : id_(id), cam_(cam), T_f_w_(T_world_cam.inverse()), key_pts_(5, std::make_pair(-1, BearingVector())), timestamp_(timestamp_ns)
  {
  }
  ...
  void Frame::setKeyPoints()
  {
    ...
    for (size_t i = 0; i < num_features_; ++i)
    {
      ...
      if (u >= cu && v < cv)
      {
        ...
        else if ((u - cu) * (v - cv) > (px_vec_(0, key_pts_[2].first) - cu) * (px_vec_(1, key_pts_[2].first) - cv))
          key_pts_[2] = std::make_pair(i, landmark_vec_[i]->pos_);
      }
      if (u < cv && v < cv)
      {
      ...
      if (u < cv && v >= cv)
      {
        ...
        else if ((u - cu) * (v - cv) > (px_vec_(0, key_pts_[4].first) - cu) * (px_vec_(1, key_pts_[4].first) - cv))
          key_pts_[4] = std::make_pair(i, landmark_vec_[i]->pos_);
      }
    }
  }
