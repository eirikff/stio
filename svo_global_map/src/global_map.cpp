...
namespace svo
{
  ...
  GlobalMap::GlobalMap(const GlobalMapOptions &options,
      ...
      : options_(options), out_((options_.print_stdcout ? std::cout : kNullOutput)), ncams_(ncams)
  {
         ...
         match_it != match_points_info_to_add_.end();)
    {
           ...
           pit != cur_match_info.pt_id_matches_.end();)
      {
        ...
        if (first_id == second_id)
        {
          ...
          pit = (all_in_graph ? cur_match_info.pt_id_matches_.erase(pit) : std::next(pit));
        }
        else
        {
      ...
      // move the next one
      match_it = (cur_match_info.pt_id_matches_.empty() ? match_points_info_to_add_.erase(match_it) : std::next(match_it));
    }
      ...
      const gtsam::Values &estimate, const BundleId last_optimized_state_index)
  {
    ...
    for (auto &v : frame_id_to_kf_in_graph_map_)
    {
      ...
      for (const auto pt_id : it_vis->second)
      {
        ...
        {
          f->type_vec_[ftr_idx] = isPointStable(pt_id) ? FeatureType::kFixedLandmark : FeatureType::kOutlier;
          continue;
        }
