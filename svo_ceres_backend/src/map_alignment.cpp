...
namespace svo
{
  MapAlignmentSE3::MapAlignmentSE3(const MapAlignmentOptions &map_alignment_options) : num_points_(0), max_num_points_(100),
  ...
  bool MapAlignmentSE3::getTransformation(Transformation &T_old_new)
  {
    ...
    for (int i = 0; i < num_points_; ++i)
    {
      W1 += (points_new_no_outliers_newit.col(i) - mean_new) * (points_old_no_outliers_newit.col(i) - mean_old).transpose();
    }
    W1 /= num_points_;
    ...
    for (int i = 0; i < num_points_; ++i)
    {
      double curr_res = (points_new_no_outliers_newit.col(i) - R1 * points_old_no_outliers_newit.col(i) - T_old_new.getPosition()).norm();
      residual += curr_res;
    }
  ...
  bool MapAlignmentSE3::getTransformRansac(Transformation &T_old_new, const int &min_num_3d, int &n_ransac_inliers, bool &relax_thresh)
  {
    ...
    std::shared_ptr<opengv::sac_problems::point_cloud::PointCloudSacProblem> relposeproblem_ptr(new opengv::sac_problems::point_cloud::PointCloudSacProblem(adapter));
