...
namespace vk
{
  ...
  inline double convertStringToDouble(const std::string &number_as_string)
  {
    ...
    {
      LOG(FATAL) << "Invalid argument exception. Could not convert '" << number_as_string << "' to a double. " << invalid_argument_exception.what();
    }
    catch (const std::out_of_range &out_of_range_exception)
    {
      LOG(FATAL) << "Out of range exception. The given number '" << number_as_string << "' does not seem to fit into a double! " << out_of_range_exception.what();
    }
    catch (const std::exception &exception)
    {
      LOG(FATAL) << "Conversion of the given number '" << number_as_string << "' to a double failed. " << exception.what();
    }
  ...
  inline int64_t convertStringToLongLong(const std::string &number_as_string)
  {
    ...
    {
      LOG(FATAL) << "Invalid argument exception. Could not convert '" << number_as_string << "' to a longlong. " << invalid_argument_exception.what();
    }
    catch (const std::out_of_range &out_of_range_exception)
    {
      LOG(FATAL) << "Out of range exception. The given number '" << number_as_string << "' does not seem to fit into a longlong! " << out_of_range_exception.what();
    }
    catch (const std::exception &exception)
    {
      LOG(FATAL) << "Conversion of the given number '" << number_as_string << "' to a longlong failed. " << exception.what();
    }
