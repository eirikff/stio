...
namespace svo
{
  ...
  bool Matcher::isPatchWithinImage(
      ...
      const int patch_level)
  {
    return !(pxi[0] < kPatchSize || pxi[1] < kPatchSize || pxi[0] >= (static_cast<int>(frame.cam()->imageWidth() / (1 << patch_level)) - kPatchSize) || pxi[1] >= (static_cast<int>(frame.cam()->imageHeight() / (1 << patch_level)) - kPatchSize));
  }
